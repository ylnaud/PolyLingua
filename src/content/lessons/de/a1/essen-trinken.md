---
language: de
level: a1
title: "Essen und Trinken: pide comida sin pasar hambre"
description: "El vocabulario de supervivencia para restaurantes, supermercados y la mesa de cualquier casa alemana."
order: 7
grammarTopic: "Vocabulario: comida y bebida"
funFact: "En un restaurante alemán, decir simplemente 'Ich möchte...' (quisiera...) antes de cualquier plato es la fórmula mágica que suena educada y natural — mucho más que 'Ich will' (quiero), que puede sonar brusco."
minutes: 6
quiz:
  - question: "¿Cuál es el artículo correcto para 'Wasser' (agua)?"
    options: ["der Wasser", "die Wasser", "das Wasser"]
    answerIndex: 2
    explanation: "'das Wasser' es neutro — como muchos líquidos y sustancias básicas en alemán."
  - question: "¿Cómo pedirías educadamente un café?"
    options: ["Ich will einen Kaffee.", "Ich möchte einen Kaffee.", "Kaffee!"]
    answerIndex: 1
    explanation: "'Ich möchte...' es la forma educada y estándar de pedir algo en un restaurante o tienda — mucho más natural que 'Ich will' (quiero, más directo/brusco)."
  - question: "¿Cuál es el artículo correcto para 'Brot' (pan)?"
    options: ["der Brot", "die Brot", "das Brot"]
    answerIndex: 2
    explanation: "'das Brot' es neutro. El pan es central en la cultura alemana — vale la pena memorizar bien su artículo."
exercises:
  - type: "fill-blank"
    sentence: "Ich ___ einen Kaffee, bitte."
    answer: "möchte"
    accepted: ["Möchte"]
    hint: "Es la forma educada de pedir algo, viene de 'mögen' — mucho más suave que 'Ich will'."
    translation: "Quisiera un café, por favor."
  - type: "fill-blank"
    sentence: "___ Brot schmeckt gut."
    answer: "Das"
    accepted: ["das"]
    hint: "'Brot' es neutro: das Brot."
    translation: "El pan sabe bien."
  - type: "match"
    instructions: "Empareja cada palabra alemana con su traducción."
    pairs:
      - left: "Käse"
        right: "el queso"
      - left: "Suppe"
        right: "la sopa"
      - left: "Milch"
        right: "la leche"
      - left: "Saft"
        right: "el jugo"
      - left: "Apfel"
        right: "la manzana"
  - type: "write"
    prompt: "Escribe la palabra alemana para 'té'."
    answer: "Tee"
    accepted: ["tee"]
    hint: "Es masculino: der Tee."
  - type: "order"
    sentence: "Ich möchte einen Kaffee bitte"
    translation: "Quisiera un café, por favor."
---

Ya sea en un restaurante, un supermercado o la mesa de una familia alemana, este
vocabulario es indispensable desde tu primera semana.

## Comida esencial

| Alemán | Español |
| --- | --- |
| das Brot | el pan |
| der Käse | el queso |
| das Ei | el huevo |
| der Apfel | la manzana |
| der Reis | el arroz |
| die Suppe | la sopa |

## Bebidas

| Alemán | Español |
| --- | --- |
| das Wasser | el agua |
| der Kaffee | el café |
| der Tee | el té |
| der Saft | el jugo |
| die Milch | la leche |

## Pedir en un restaurante

```
Ich möchte einen Kaffee, bitte.       (Quisiera un café, por favor)
Die Rechnung, bitte.                   (La cuenta, por favor)
Was empfehlen Sie?                     (¿Qué recomienda?)
```

## "möchte": tu palabra más útil aquí

"Ich möchte" (quisiera) es la forma cortés de "querer" — literalmente el Konjunktiv II de
"mögen" que verás formalmente más adelante, pero que puedes usar ya mismo como fórmula
fija. Es la palabra que hace que pedir comida suene natural en vez de brusco.
