---
language: de
level: a1
title: "Die Familie: presenta a los tuyos en alemán"
description: "Vocabulario de la familia en alemán: cómo nombrar a padres, hermanos, abuelos y demás parientes usando el patrón der/die predecible."
order: 8
grammarTopic: "Vocabulario: la familia"
funFact: "Casi todas las palabras de familia masculina y femenina siguen el patrón der/die predecible: der Vater, die Mutter, der Bruder, die Schwester. Apréndelas en parejas (padre/madre, hermano/hermana) y el vocabulario se duplica con la mitad del esfuerzo."
minutes: 6
quiz:
  - question: "¿Cuál es el artículo correcto para 'Mutter' (madre)?"
    options: ["der Mutter", "die Mutter", "das Mutter"]
    answerIndex: 1
    explanation: "'die Mutter' — todas las personas de sexo femenino son femeninas en alemán, sin excepción."
  - question: "¿Cómo se dice 'hermanos' (varios, sin especificar género) en alemán?"
    options: ["die Brüder", "die Geschwister", "die Schwestern"]
    answerIndex: 1
    explanation: "'die Geschwister' es la palabra colectiva para 'hermanos y hermanas' juntos, sin especificar género — muy usada al hablar de familia."
  - question: "¿Cuál es el artículo correcto para 'Kind' (niño/niña)?"
    options: ["der Kind", "die Kind", "das Kind"]
    answerIndex: 2
    explanation: "'das Kind' es neutro — una de las pocas palabras de persona que no sigue el patrón masculino/femenino según el sexo real."
exercises:
  - type: "fill-blank"
    sentence: "Meine ___ heißt Ana."
    answer: "Mutter"
    accepted: ["mutter"]
    hint: "La persona que te dio la vida: die Mutter."
    translation: "Mi madre se llama Ana."
  - type: "fill-blank"
    sentence: "Ich habe zwei ___."
    answer: "Geschwister"
    accepted: ["geschwister"]
    hint: "Palabra colectiva para hermanos y hermanas juntos, sin especificar género."
    translation: "Tengo dos hermanos."
  - type: "match"
    instructions: "Empareja cada palabra alemana con su traducción."
    pairs:
      - left: "Vater"
        right: "el padre"
      - left: "Tochter"
        right: "la hija"
      - left: "Onkel"
        right: "el tío"
      - left: "Cousine"
        right: "la prima"
      - left: "Geschwister"
        right: "los hermanos (colectivo)"
  - type: "write"
    prompt: "Escribe la palabra alemana para 'abuelo'."
    answer: "Großvater"
    accepted: ["großvater", "Grossvater", "grossvater"]
    hint: "Es una palabra compuesta: groß (grande) + Vater (padre)."
    spokenOnly: true
  - type: "order"
    sentence: "Mein Bruder ist 20 Jahre alt"
    translation: "Mi hermano tiene 20 años."
---

Presentar a tu familia es una de las primeras conversaciones reales que vas a tener en
alemán. Aquí tienes el vocabulario esencial.

## La familia nuclear

| Alemán | Español |
| --- | --- |
| die Familie | la familia |
| der Vater / die Mutter | el padre / la madre |
| die Eltern | los padres |
| der Sohn / die Tochter | el hijo / la hija |
| der Bruder / die Schwester | el hermano / la hermana |
| die Geschwister | los hermanos (colectivo) |

## La familia extendida

| Alemán | Español |
| --- | --- |
| die Großmutter / der Großvater | la abuela / el abuelo |
| die Tante / der Onkel | la tía / el tío |
| die Cousine / der Cousin | la prima / el primo |

## Hablar de tu familia

```
Ich habe zwei Geschwister.        (Tengo dos hermanos)
Meine Mutter heißt Ana.            (Mi madre se llama Ana)
Mein Bruder ist 20 Jahre alt.      (Mi hermano tiene 20 años)
```

## El truco de las parejas

Aprende cada palabra junto a su contraparte de género: Vater/Mutter, Bruder/Schwester,
Sohn/Tochter. Ese emparejamiento hace que memorices el doble de vocabulario sin doble
esfuerzo, y refuerza el patrón der/die según el sexo de la persona.
