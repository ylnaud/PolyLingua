---
language: de
level: a2
title: "Pläne machen: invitar, aceptar y cancelar"
description: "Hast du Lust ins Kino zu gehen, das passt mir gut, ich muss leider absagen: las frases para arreglar planes con alguien en alemán."
order: 17
unit: 4
grammarTopic: "Frases para hacer planes y quedar con alguien"
situation: "Hacer planes"
funFact: "«Wollen wir ins Kino gehen?» no es «¿queremos ir al cine?» sino «¿vamos al cine?». En alemán, «wollen wir...?» es la forma normal de proponer algo, igual que el «¿vamos...?» del español. Traducirlo literal confunde; usarlo como bloque, no."
minutes: 9
phrases:
  - text: "Hast du am Samstag Zeit?"
    es: "¿Tenés tiempo el sábado?"
  - text: "Wollen wir ins Kino gehen?"
    es: "¿Vamos al cine?"
    note: "«Wollen wir...?» es la forma normal de proponer algo."
  - text: "Hast du Lust, einen Kaffee zu trinken?"
    es: "¿Tenés ganas de tomar un café?"
    note: "«Lust haben, ... zu» + infinitivo al final."
  - text: "Ja, gern!"
    es: "¡Sí, con gusto!"
  - text: "Das passt mir gut."
    es: "Me viene bien."
    note: "«passen» rige Dativ: das passt mir, dir, ihm."
  - text: "Das passt mir leider nicht."
    es: "Lamentablemente no me viene bien."
  - text: "Wann treffen wir uns?"
    es: "¿Cuándo nos encontramos?"
  - text: "Um acht Uhr vor dem Kino."
    es: "A las ocho frente al cine."
  - text: "Abgemacht!"
    es: "¡Trato hecho!"
  - text: "Ich muss leider absagen."
    es: "Lamentablemente tengo que cancelar."
  - text: "Vielleicht ein anderes Mal."
    es: "Quizás en otro momento."
  - text: "Wir sehen uns morgen."
    es: "Nos vemos mañana."
quiz:
  - question: "«Wollen wir ins Kino gehen?» significa..."
    options: ["¿Queremos ir al cine?", "¿Vamos al cine?", "¿Querés que vaya al cine?"]
    answerIndex: 1
    explanation: "Traducido literal suena raro, pero «Wollen wir...?» es simplemente la forma de proponer un plan, igual que el «¿vamos...?» del español."
  - question: "«Das passt mir gut» — ¿por qué «mir» y no «mich»?"
    options:
      - "Porque «passen» rige Dativ."
      - "Porque siempre se usa «mir» con «gut»."
      - "Es un error, debería ser «mich»."
    answerIndex: 0
    explanation: "«passen» es de los verbos que piden Dativ, igual que «helfen» y «gefallen»: das passt mir, dir, ihm."
  - question: "En «Hast du Lust, einen Kaffee zu trinken?», el infinitivo va..."
    options: ["Justo después de «Lust».", "Al final, con «zu» delante.", "Antes del sujeto."]
    answerIndex: 1
    explanation: "La estructura es «Lust haben, ... zu + infinitivo», y el infinitivo cierra la frase."
exercises:
  - type: "fill-blank"
    sentence: "Das passt ___ gut."
    answer: "mir"
    accepted: ["Mir"]
    hint: "«passen» rige Dativ."
    translation: "Me viene bien."
    placeholder: "mich / mir"
  - type: "fill-blank"
    sentence: "___ wir ins Kino gehen?"
    answer: "Wollen"
    accepted: ["wollen"]
    hint: "La forma de proponer un plan."
    translation: "¿Vamos al cine?"
    placeholder: "Wollen / Haben"
  - type: "match"
    instructions: "Empareja cada frase con su significado."
    pairs:
      - left: "Hast du am Samstag Zeit?"
        right: "¿Tenés tiempo el sábado?"
      - left: "Das passt mir gut"
        right: "Me viene bien"
      - left: "Ich muss leider absagen"
        right: "Tengo que cancelar"
      - left: "Abgemacht!"
        right: "¡Trato hecho!"
  - type: "write"
    prompt: "«Hast du am Samstag Zeit?» — Respondé que sí y que te viene bien."
    answer: "Ja, das passt mir gut"
    accepted: ["Ja, das passt mir gut.", "Das passt mir gut", "Das passt mir gut."]
    hint: "«passen» + Dativ."
    placeholder: "Ja, das passt ..."
  - type: "write"
    prompt: "Quedaste con alguien pero no podés ir. Decí que lamentablemente tenés que cancelar."
    answer: "Ich muss leider absagen"
    accepted: ["Ich muss leider absagen.", "Leider muss ich absagen", "Ich muss absagen"]
    hint: "«leider» = lamentablemente. Con modal, el infinitivo va al final."
    placeholder: "Ich muss ..."
  - type: "order"
    sentence: "Wann treffen wir uns morgen"
    translation: "¿Cuándo nos encontramos mañana?"
vocabulary:
  - term: "Lust haben"
    translation: "tener ganas"
  - term: "passen"
    translation: "venir bien (rige Dativ)"
  - term: "sich treffen"
    translation: "encontrarse"
  - term: "absagen"
    translation: "cancelar"
  - term: "leider"
    translation: "lamentablemente"
  - term: "abgemacht"
    translation: "trato hecho"
---

Arreglar un plan tiene siempre los mismos cuatro pasos: proponer, aceptar o rechazar,
acordar la hora, y a veces cancelar.

## 1. Proponer

```
Hast du am Samstag Zeit?                     ¿Tenés tiempo el sábado?
Wollen wir ins Kino gehen?                   ¿Vamos al cine?
Hast du Lust, einen Kaffee zu trinken?       ¿Tenés ganas de tomar un café?
```

**`Wollen wir...?`** traducido literal sería «¿queremos...?», que suena rarísimo. Pero
en alemán es simplemente la forma de proponer algo — el equivalente exacto de nuestro
«¿vamos...?». Usalo como bloque y listo.

La otra fórmula, `Hast du Lust, ... zu + infinitivo`, manda el verbo al final:

```
Hast du Lust,  einen Kaffee  zu trinken?
```

## 2. Aceptar o rechazar

```
Ja, gern!                        ¡Sí, con gusto!
Das passt mir gut.               Me viene bien.
Das passt mir leider nicht.      Lamentablemente no me viene bien.
Vielleicht ein anderes Mal.      Quizás en otro momento.
```

`passen` es otro verbo que rige **Dativ**, igual que `helfen` y `gefallen`: `das passt
**mir**`, no *"das passt mich"*.

Y `leider` (lamentablemente) es la palabra que suaviza cualquier negativa. Un `nein` a
secas suena cortante; `leider nicht` es lo normal.

## 3. Acordar

```
Wann treffen wir uns?            ¿Cuándo nos encontramos?
Um acht Uhr vor dem Kino.        A las ocho frente al cine.
Abgemacht!                       ¡Trato hecho!
```

## 4. Cancelar

```
Ich muss leider absagen.         Lamentablemente tengo que cancelar.
Wir sehen uns morgen.            Nos vemos mañana.
```

## El truco para practicar

Ensayá el intercambio completo en voz alta, haciendo los dos papeles: uno propone, el
otro acepta, acuerdan hora. Después repetilo pero rechazando con `Das passt mir leider
nicht`. Son seis frases que cubren cualquier plan que quieras arreglar.
