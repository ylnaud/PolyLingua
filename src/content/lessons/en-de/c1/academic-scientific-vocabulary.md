---
language: 'de'
level: 'c1'
title: 'German Academic and Scientific Vocabulary'
description: 'Build core German academic and scientific vocabulary — These, Hypothese, Ergebnis, untersuchen, belegen — for reading and writing research texts at C1.'
order: 8
unit: 2
grammarTopic: 'Academic/scientific language vocabulary'
funFact: "German academic writing runs on a small set of workhorse verbs — untersuchen, belegen, widerlegen, nachweisen — that you'll meet in nearly every research abstract. Learn these eight or ten verbs cold and you can already follow the skeleton of almost any German study, even before you know a single word of its specific subject matter."
skills:
  - de.c1.vocabulary.academic
minutes: 10
quiz:
  - question: 'What does "die These" mean in academic German?'
    options:
      [
        'thesis/claim (a proposed statement to be argued or tested)',
        'a footnote',
        'a university degree',
      ]
    answerIndex: 0
    explanation: '"die These" is a thesis or claim — the central statement an academic text sets out to argue, support, or test.'
  - question: 'What is the difference between "die These" and "die Hypothese"?'
    options:
      [
        'They are unrelated concepts from different fields',
        'A These is a broader claim/argument; a Hypothese is a specific, testable prediction, often in empirical research',
        'Hypothese is only used in literature studies',
      ]
    answerIndex: 1
    explanation: 'die These is a general claim or argument; die Hypothese is narrower — a specific, testable prediction, typically used in empirical/scientific research design.'
  - question: 'What does "untersuchen" mean, and why is it central to academic vocabulary?'
    options:
      [
        'to publish',
        'to investigate/examine — the core verb for describing what a study does',
        'to reject',
      ]
    answerIndex: 1
    explanation: '"untersuchen" (to investigate, examine, study) is one of the most common verbs in academic writing, describing what a study, researcher, or method does to a topic.'
  - question: 'What does "belegen" mean in an academic context?'
    options: ['to support with evidence, to substantiate', 'to travel to', 'to translate']
    answerIndex: 0
    explanation: '"belegen" means to support a claim with evidence/proof: Die Daten belegen diese These (The data support this thesis).'
  - question: 'What does "widerlegen" mean?'
    options: ['to confirm', 'to refute, disprove', 'to summarize']
    answerIndex: 1
    explanation: '"widerlegen" means to refute or disprove — the opposite of belegen: Die neuen Ergebnisse widerlegen die alte Hypothese (The new results refute the old hypothesis).'
  - question: 'What does "signifikant" mean in a scientific/statistical context?'
    options:
      [
        'loud, noticeable to the ear',
        'statistically significant (not due to chance)',
        'famous, well-known',
      ]
    answerIndex: 1
    explanation: '"signifikant" in scientific German specifically means statistically significant: ein signifikanter Unterschied (a statistically significant difference).'
  - question: 'What does "empirisch" describe?'
    options:
      [
        'based on observation/experiment/data rather than theory alone',
        'purely theoretical, with no data',
        'related to emotions',
      ]
    answerIndex: 0
    explanation: '"empirisch" (empirical) describes research based on observed, measured, or experimental data, as opposed to purely theoretical (theoretisch) work.'
  - question: 'Which sentence correctly uses academic vocabulary to say "The study confirms the hypothesis"?'
    options:
      [
        'Die Studie bestätigt die Hypothese.',
        'Die Studie widerspricht die Hypothese.',
        'Die Studie fragt die Hypothese.',
      ]
    answerIndex: 0
    explanation: '"bestätigen" means to confirm: Die Studie bestätigt die Hypothese (The study confirms the hypothesis) — the correct, standard academic phrasing.'
exercises:
  - type: 'fill-blank'
    sentence: 'Die neuen Daten ___ die ursprüngliche Hypothese nicht — sie widersprechen ihr sogar. (belegen, verneint)'
    answer: 'belegen'
    accepted: ['belegen']
    hint: 'belegen = to support with evidence; here it is negated: die Daten belegen die Hypothese NICHT.'
    translation: 'The new data does not support the original hypothesis — it even contradicts it.'
    placeholder: 'belegen / widerlegen / fragen'
  - type: 'fill-blank'
    sentence: 'Der Forscher konnte die alte Theorie klar ___. (widerlegen)'
    answer: 'widerlegen'
    accepted: ['widerlegen']
    hint: 'widerlegen = to refute/disprove — the opposite of belegen (to support with evidence).'
    translation: 'The researcher was able to clearly refute the old theory.'
    placeholder: 'widerlegen / belegen / übersetzen'
  - type: 'match'
    instructions: 'Match each academic term with its English meaning.'
    pairs:
      - left: 'die These'
        right: 'thesis, claim'
      - left: 'die Hypothese'
        right: 'hypothesis'
      - left: 'untersuchen'
        right: 'to investigate'
      - left: 'belegen'
        right: 'to support with evidence'
      - left: 'widerlegen'
        right: 'to refute'
      - left: 'signifikant'
        right: 'statistically significant'
  - type: 'write'
    prompt: 'Write "The study confirms the hypothesis" using bestätigen.'
    answer: 'Die Studie bestätigt die Hypothese.'
    accepted: ['Die Studie bestätigt die Hypothese']
    hint: 'bestätigen (to confirm) + die Hypothese (accusative object, same form as nominative here).'
    placeholder: 'Die Studie bestätigt ...'
  - type: 'write'
    prompt: 'Write "The results are statistically significant" using signifikant.'
    answer: 'Die Ergebnisse sind statistisch signifikant.'
    accepted: ['Die Ergebnisse sind statistisch signifikant']
    hint: 'die Ergebnisse (results) + sein (to be) + statistisch signifikant (statistically significant, adverb + adjective).'
    placeholder: 'Die Ergebnisse sind ...'
  - type: 'order'
    sentence: 'Die Daten belegen die zentrale These der Studie'
    translation: 'The data support the central thesis of the study.'
vocabulary:
  - term: 'die These'
    translation: 'thesis, claim'
  - term: 'die Hypothese'
    translation: 'hypothesis'
  - term: 'das Ergebnis'
    translation: 'result, finding'
  - term: 'die Schlussfolgerung'
    translation: 'conclusion'
  - term: 'untersuchen'
    translation: 'to investigate, examine'
  - term: 'belegen'
    translation: 'to support with evidence, substantiate'
  - term: 'widerlegen'
    translation: 'to refute, disprove'
  - term: 'nachweisen'
    translation: 'to prove, demonstrate'
  - term: 'empirisch'
    translation: 'empirical'
  - term: 'signifikant'
    translation: 'significant (statistically)'
  - term: 'die Studie'
    translation: 'study'
  - term: 'der Forscher / die Forscherin'
    translation: 'researcher'
---

Reading a German research paper, abstract, or academic article requires a specific vocabulary
set that rarely appears in everyday conversation. Fortunately, this vocabulary is small,
recurring, and highly learnable — a manageable core of nouns and verbs that structure almost any
academic text.

## Core nouns: the building blocks of an argument

| Term                              | Meaning                                            |
| --------------------------------- | -------------------------------------------------- |
| **die These**                     | thesis, claim — the central statement being argued |
| **die Hypothese**                 | hypothesis — a specific, testable prediction       |
| **das Ergebnis**                  | result, finding                                    |
| **die Schlussfolgerung**          | conclusion                                         |
| **die Studie**                    | study                                              |
| **die Methode**                   | method                                             |
| **das Experiment**                | experiment                                         |
| **der Forscher / die Forscherin** | researcher                                         |
| **die Quelle**                    | source                                             |
| **der Beleg**                     | (piece of) evidence                                |

Note the distinction between **These** and **Hypothese**: a _These_ is a broader claim or
position an author argues for; a _Hypothese_ is narrower and typically tied to empirical
research design — something you test and can confirm or refute with data.

## Core verbs: what a study _does_

A small set of verbs describes almost every move an academic text makes:

- **untersuchen** — to investigate, examine: _Die Studie untersucht den Einfluss von..._
- **analysieren** — to analyze
- **belegen** — to support with evidence, substantiate: _Die Daten belegen diese These._
- **widerlegen** — to refute, disprove: _Die Ergebnisse widerlegen die alte Theorie._
- **bestätigen** — to confirm: _Die Studie bestätigt die Hypothese._
- **nachweisen** — to prove, demonstrate (often with hard data): _Der Versuch weist den Effekt
  nach._
- **feststellen** — to determine, establish, find (as a result): _Die Autoren stellten fest,
  dass..._
- **zeigen** — to show

**belegen** and **widerlegen** are natural opposites worth learning as a pair — a study either
supports or undermines a given claim, and these are the two verbs that say so.

## Key adjectives

- **empirisch** — empirical, based on observation/data (vs. **theoretisch**, purely theoretical)
- **signifikant** — statistically significant (a precise, technical meaning, not just "notable")
- **repräsentativ** — representative (of a sample vs. a population)
- **valide** — valid (methodologically sound)
- **objektiv / subjektiv** — objective / subjective
- **plausibel** — plausible

## A sample academic sentence, decoded

> **Die vorliegende Studie untersucht, ob sich die neue Methode empirisch belegen lässt. Die
> Ergebnisse bestätigen die zentrale Hypothese und widerlegen gleichzeitig frühere Annahmen.**

(This study investigates whether the new method can be empirically substantiated. The results
confirm the central hypothesis and simultaneously refute earlier assumptions.)

Notice how much of the sentence's meaning rides on just a handful of high-frequency academic
verbs — _untersuchen_, _belegen_, _bestätigen_, _widerlegen_ — around a core noun, _die
Hypothese_. Once these words are automatic, the specific subject matter of almost any German
research text becomes far easier to follow, because the scaffolding around it is always built
from the same small vocabulary set.
