---
language: 'de'
level: 'c1'
title: 'German Functional Verb Constructions (Funktionsverbgefüge)'
description: 'Learn Funktionsverbgefüge — formal German phrases like in Kraft treten and zur Verfügung stehen — that replace simple verbs in bureaucratic and business German.'
order: 4
unit: 1
grammarTopic: 'Functional verb constructions (Funktionsverbgefüge)'
funFact: 'A Funktionsverbgefüge is a verb wearing a disguise: "eine Entscheidung treffen" doesn''t add anything "entscheiden" doesn''t already say — it just sounds more official. Spot the pattern (weak verb + noun, often with a fixed preposition) and you can decode almost any bureaucratic or business sentence by mentally collapsing it back into the one simple verb hiding inside.'
skills:
  - de.c1.construction.funktionsverb
minutes: 10
quiz:
  - question: 'What is a Funktionsverbgefüge (functional verb construction)?'
    options:
      [
        'A modal verb combined with an infinitive',
        'A semantically weak verb + a noun that together carry the main meaning, replacing a simple verb',
        'A separable verb with a detachable prefix',
      ]
    answerIndex: 1
    explanation: 'A Funktionsverbgefüge pairs a "light" verb (e.g. treffen, stehen, finden) with a noun that carries the real meaning: eine Entscheidung treffen = entscheiden (to decide).'
  - question: 'Which simple verb does "eine Entscheidung treffen" correspond to?'
    options: ['entscheiden', 'entwerfen', 'entstehen']
    answerIndex: 0
    explanation: '"eine Entscheidung treffen" (to make a decision) is a Funktionsverbgefüge for the simple verb "entscheiden" (to decide).'
  - question: 'What does "zur Verfügung stehen" mean?'
    options: ['to be in effect (of a law)', 'to be available', 'to be under discussion']
    answerIndex: 1
    explanation: '"zur Verfügung stehen" (literally "to stand at disposal") means "to be available": Das Team steht Ihnen zur Verfügung (The team is available to you).'
  - question: 'What does "in Kraft treten" mean, typically said of laws or contracts?'
    options: ['to be discussed', 'to take effect / come into force', 'to be cancelled']
    answerIndex: 1
    explanation: '"in Kraft treten" (literally "to step into force") means a law or regulation takes effect: Das Gesetz tritt am 1. Januar in Kraft.'
  - question: 'Which register is Funktionsverbgefüge most typical of?'
    options:
      [
        'casual spoken German among friends',
        'formal, bureaucratic, legal, and business German',
        'children’s books',
      ]
    answerIndex: 1
    explanation: 'Funktionsverbgefüge are a hallmark of formal/bureaucratic/business/legal register — official documents, news, contracts, and administrative language.'
  - question: 'What does "Kritik üben an + Dativ" mean?'
    options:
      ['to practice criticism as a hobby', 'to criticize (someone/something)', 'to avoid criticism']
    answerIndex: 1
    explanation: '"Kritik üben an + Dativ" is the Funktionsverbgefüge for "kritisieren": Er übte Kritik an der Entscheidung = Er kritisierte die Entscheidung.'
  - question: 'What does "Anwendung finden" mean?'
    options: ['to be found by accident', 'to be applied / to be used', 'to disappear']
    answerIndex: 1
    explanation: '"Anwendung finden" is the Funktionsverbgefüge for "angewendet werden" (to be applied): Diese Methode findet in vielen Bereichen Anwendung.'
  - question: 'Why do writers of formal German prefer Funktionsverbgefüge over the simple verb, even though they mean the same thing?'
    options:
      [
        'They allow adding adjectives to the noun and sound more objective/impersonal, fitting formal register',
        'They are always shorter to write',
        'They are required by German spelling rules',
      ]
    answerIndex: 0
    explanation: 'Funktionsverbgefüge let writers modify the noun (eine schwierige Entscheidung treffen) and create a more impersonal, official tone — both prized in bureaucratic and academic writing.'
exercises:
  - type: 'fill-blank'
    sentence: 'Das neue Gesetz tritt am 1. Januar in ___. (in Kraft treten)'
    answer: 'Kraft'
    accepted: ['Kraft']
    hint: 'The fixed noun in this Funktionsverbgefüge is "Kraft": in Kraft treten = to take effect.'
    translation: 'The new law takes effect on January 1st.'
    placeholder: 'Kraft / Wirkung / Macht'
  - type: 'fill-blank'
    sentence: 'Unser Support-Team steht Ihnen jederzeit zur ___. (zur Verfügung stehen)'
    answer: 'Verfügung'
    accepted: ['Verfügung']
    hint: 'The fixed noun here is "Verfügung": zur Verfügung stehen = to be available.'
    translation: 'Our support team is available to you at any time.'
    placeholder: 'Verfügung / Hilfe / Seite'
  - type: 'match'
    instructions: 'Match each Funktionsverbgefüge with its simple-verb equivalent.'
    pairs:
      - left: 'eine Entscheidung treffen'
        right: 'entscheiden'
      - left: 'Kritik üben an'
        right: 'kritisieren'
      - left: 'Anwendung finden'
        right: 'angewendet werden'
      - left: 'in Kraft treten'
        right: 'gelten (ab einem Datum)'
      - left: 'zur Verfügung stehen'
        right: 'verfügbar sein'
  - type: 'write'
    prompt: 'Rewrite using a Funktionsverbgefüge instead of the simple verb: "Der Vorstand entscheidet morgen." (use "eine Entscheidung treffen")'
    answer: 'Der Vorstand trifft morgen eine Entscheidung.'
    accepted: ['Der Vorstand trifft morgen eine Entscheidung']
    hint: 'Swap "entscheidet" for "trifft eine Entscheidung" — the noun (Entscheidung) carries the meaning, treffen is the light verb.'
    placeholder: 'Der Vorstand trifft ...'
  - type: 'write'
    prompt: 'Express "The professor criticized the study" using "Kritik üben an + Dativ".'
    answer: 'Der Professor übte Kritik an der Studie.'
    accepted: ['Der Professor übte Kritik an der Studie']
    hint: '"Kritik üben an + Dativ" replaces "kritisieren": an der Studie (Dativ, feminine "die Studie").'
    placeholder: 'Der Professor übte Kritik an ...'
  - type: 'order'
    sentence: 'Das Gesetz tritt am ersten Januar in Kraft'
    translation: 'The law takes effect on January 1st.'
vocabulary:
  - term: 'das Funktionsverbgefüge'
    translation: 'functional verb construction'
  - term: 'eine Entscheidung treffen'
    translation: 'to make a decision'
  - term: 'zur Verfügung stehen'
    translation: 'to be available'
  - term: 'in Kraft treten'
    translation: 'to take effect, come into force'
  - term: 'Kritik üben an'
    translation: 'to criticize'
  - term: 'Anwendung finden'
    translation: 'to be applied / used'
  - term: 'zum Ausdruck bringen'
    translation: 'to express'
---

Read a German contract, a company press release, or a law, and you will notice something odd:
verbs like _entscheiden_ (to decide) or _kritisieren_ (to criticize) rarely appear. Instead you
get _eine Entscheidung treffen_ (to make a decision) and _Kritik üben_ (to exercise criticism).
These are **Funktionsverbgefüge** — functional verb constructions — and they are the fingerprint
of formal, bureaucratic, and business German.

## The pattern: a "light" verb + a noun

A Funktionsverbgefüge pairs a semantically weak (or "light") verb — often _treffen_, _stehen_,
_bringen_, _finden_, _üben_, _nehmen_, _geben_ — with a noun that carries the real meaning,
sometimes with a fixed preposition:

| Funktionsverbgefüge             | Simple verb             | Meaning              |
| ------------------------------- | ----------------------- | -------------------- |
| eine Entscheidung **treffen**   | entscheiden             | to decide            |
| zur Verfügung **stehen**        | verfügbar sein          | to be available      |
| in Kraft **treten**             | gelten (ab einem Datum) | to take effect       |
| Kritik üben **an** + Dat.       | kritisieren             | to criticize         |
| Anwendung **finden**            | angewendet werden       | to be applied        |
| zum Ausdruck **bringen**        | ausdrücken              | to express           |
| eine Frage **stellen**          | fragen                  | to ask a question    |
| Rücksicht **nehmen auf** + Akk. | berücksichtigen         | to take into account |

The verb itself (treffen, stehen, bringen...) contributes almost no independent meaning here —
its job is purely grammatical, to carry tense and person while the noun does the real semantic
work.

## Why German prefers these in formal writing

Three concrete advantages explain why Funktionsverbgefüge are everywhere in official language:

1. **You can modify the noun with adjectives**, which you cannot do to a simple verb: _eine
   schwierige Entscheidung treffen_ (to make a difficult decision) — there is no equivalent way
   to insert "difficult" into the verb _entscheiden_ itself.
2. **They sound more impersonal and objective** — a favored tone in laws, contracts, and
   academic writing, where the action is framed as a formal, almost bureaucratic event rather
   than a personal choice.
3. **They allow nominalization patterns** to interlock smoothly with the rest of formal German's
   heavy use of nouns (see the Nominalstil lesson) — the noun inside the construction can itself
   be modified, referenced, or turned into a genitive phrase.

## Learning them as fixed units

Because the light verb + noun + preposition combination is largely fixed (you cannot swap
_treffen_ for _machen_ in _eine Entscheidung treffen_), the only reliable way to master
Funktionsverbgefüge is to **memorize each one as a chunk**, the same way you would memorize an
English idiom like "to make a decision" (not _"to do a decision"_).

A few more high-frequency examples worth memorizing directly:

- **eine Rolle spielen** — to play a role
- **Rücksicht nehmen auf** — to take into consideration
- **einen Beitrag leisten zu** — to contribute to
- **unter Beweis stellen** — to prove, to demonstrate
- **zum Einsatz kommen** — to be used/deployed

## Recognizing them while reading

The fastest way to decode a Funktionsverbgefüge you don't recognize: find the noun (it is
usually capitalized and often derived from a verb you already know — Entscheidung from
entscheiden, Anwendung from anwenden), and mentally collapse the whole phrase back into that
underlying simple verb. Once you have that habit, dense administrative and business German
becomes dramatically easier to parse.
