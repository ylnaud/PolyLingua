---
language: en
level: c1
title: 'Reported Speech: cuenta lo que otros dijeron sin comillas'
description: 'El backshift del Reported Speech en inglés, el cambio de tiempos verbales al contar lo que otros dijeron, explicado como una regla mecánica simple.'
order: 1
unit: 1
grammarTopic: 'Reported Speech (discurso indirecto)'
funFact: "Imagina que cada vez que reportas lo que alguien dijo, el tiempo verbal 'retrocede un casillero' en una línea de tiempo: presente → pasado, pasado → pasado perfecto, will → would. Es un juego de mesa donde todas las fichas se mueven una casilla hacia atrás al mismo tiempo. Ese movimiento se llama 'backshift'."
minutes: 11
quiz:
  - question: "María dijo: 'I am tired.' ¿Cómo lo reportas?"
    options: ['She said she is tired.', 'She said she was tired.', 'She said she has been tired.']
    answerIndex: 1
    explanation: "El presente ('am') retrocede a pasado ('was') al reportar el discurso: She said she was tired."
  - question: "Tom dijo: 'I will call you.' ¿Cómo lo reportas?"
    options: ['He said he will call me.', 'He said he would call me.', 'He said he calls me.']
    answerIndex: 1
    explanation: "'will' retrocede a 'would' en discurso indirecto: He said he would call me."
  - question: '¿Cuándo NO aplica el backshift (el tiempo verbal se queda igual)?'
    options:
      [
        'Nunca, siempre retrocede',
        'Cuando el hecho reportado sigue siendo verdad o es una ley general',
        "Solo con el verbo 'to be'",
      ]
    answerIndex: 1
    explanation: "Si el hecho es una verdad general o sigue siendo cierto, puedes mantener el tiempo original: She said the sun rises in the east (sigue siendo verdad, no es obligatorio cambiarlo a 'rose')."
exercises:
  - type: 'fill-blank'
    sentence: 'She said she ___ tired.'
    answer: 'was'
    hint: 'María dijo I am tired — el presente retrocede a pasado en discurso indirecto.'
    translation: 'Ella dijo que estaba cansada.'
    placeholder: 'were'
  - type: 'fill-blank'
    sentence: 'He said he ___ call me.'
    answer: 'would'
    hint: 'Tom dijo I will call you — will retrocede a would.'
    translation: 'Él dijo que me llamaría.'
    placeholder: 'could'
  - type: 'match'
    instructions: 'Empareja cada forma del discurso directo con su equivalente en discurso indirecto (backshift).'
    pairs:
      - left: 'am'
        right: 'was'
      - left: 'will'
        right: 'would'
      - left: 'can'
        right: 'could'
      - left: 'now'
        right: 'then'
      - left: 'tomorrow'
        right: 'the next day'
  - type: 'write'
    prompt: 'Reporta en discurso indirecto lo que dijo Ana: I will finish it tomorrow. Empieza con She said...'
    answer: 'She said she would finish it the next day.'
    hint: 'Aplica el backshift: will → would, y cambia el marcador temporal tomorrow → the next day.'
    placeholder: 'He said he would help me the next day.'
  - type: 'order'
    sentence: 'He said he would help me'
    translation: 'Él dijo que me ayudaría.'
vocabulary:
  - term: 'backshift'
    translation: 'retroceso de tiempos verbales'
  - term: 'will'
    translation: 'will (futuro)'
  - term: 'would'
    translation: 'would (forma reportada de will)'
  - term: 'tomorrow'
    translation: 'mañana'
  - term: 'the next day'
    translation: 'al día siguiente'
---

Cuando cuentas lo que alguien más dijo, el inglés no usa comillas como el español — usa un
cambio sistemático de tiempos verbales llamado **backshift** ("retroceso").

## La regla del backshift: un paso hacia atrás

| Discurso directo   | Discurso indirecto |
| ------------------ | ------------------ |
| Present Simple     | Past Simple        |
| Present Continuous | Past Continuous    |
| Past Simple        | Past Perfect       |
| will               | would              |
| can                | could              |
| must               | had to             |

```
Directo:    "I am hungry."        →  Indirecto: She said she was hungry.
Directo:    "I will help you."    →  Indirecto: He said he would help me.
```

## Otros cambios que acompañan al backshift

No solo el verbo cambia — también los marcadores de tiempo y lugar:

| Directo  | Indirecto    |
| -------- | ------------ |
| now      | then         |
| today    | that day     |
| tomorrow | the next day |
| here     | there        |
| this     | that         |

```
Directo:    "I'll finish it tomorrow."
Indirecto:  He said he would finish it the next day.
```

## La excepción que suena a nivel avanzado

Cuando lo dicho sigue siendo verdad, cierto hoy, o es una regla general, un hablante
avanzado **puede** saltarse el backshift:

```
"The Earth is round." → She said the Earth is round. (o "was round", ambas son válidas)
```

Reconocer cuándo puedes saltarte esta regla —sin sonar incorrecto— es exactamente lo que
distingue un inglés de nivel C1 de uno mecánico y sobre-corregido.
