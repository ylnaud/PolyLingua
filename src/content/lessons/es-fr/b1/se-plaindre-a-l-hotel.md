---
language: fr
level: b1
title: "Se plaindre à l'hôtel en français B1 : l'imparfait après « si »"
description: "Apprends à te plaindre poliment à la réception d'un hôtel en français, avec l'imparfait après « si » pour exprimer politesse et hypothèse."
order: 11
unit: 3
grammarTopic: 'Imparfait après « si » (politesse et hypothèse)'
funFact: "« si c'était possible » ne parle pas du passé — ça parle d'une hypothèse polie. Retiens-le comme un bloc figé : « si + imparfait » = demande douce, jamais une histoire ancienne."
skills:
  - fr.b1.conditional.si-imparfait
minutes: 8
quiz:
  - question: "Quelle est la forme correcte de l'imparfait de « être » à la première personne du singulier ?"
    options: ['je suis', "j'étais", 'je serai']
    answerIndex: 1
    explanation: "« étais » est l'imparfait de « être » — utilisé après « si » pour la politesse et l'hypothèse, pas pour raconter un fait passé isolé."
  - question: "« J'aimerais une autre chambre, si c'était possible » sonne comme..."
    options: ['un ordre direct', 'une demande polie', 'une phrase au passé composé']
    answerIndex: 1
    explanation: "« si c'était possible » adoucit la demande — c'est beaucoup plus poli que « je veux une autre chambre »."
  - question: "Dans « si nous avions une chambre libre », que signifie l'imparfait après « si » ?"
    options:
      [
        'que la chambre libre existe vraiment maintenant',
        'que la condition est hypothétique, probablement pas réelle',
        "que c'est arrivé hier",
      ]
    answerIndex: 1
    explanation: "« si » + imparfait exprime une condition irréelle ou peu probable au présent — ici, la réceptionniste sous-entend qu'il n'y a pas de chambre libre."
exercises:
  - type: 'fill-blank'
    sentence: "J'aimerais une autre chambre, si c'___ possible."
    answer: 'était'
    accepted: ['etait']
    hint: '« si » + imparfait pour une demande polie.'
    translation: 'Me gustaría otra habitación, si fuera posible.'
    placeholder: 'est / était / sera'
  - type: 'write'
    prompt: 'Traduis : « Disculpe, lamentablemente tengo un problema con mi habitación. »'
    answer: "Excusez-moi, j'ai malheureusement un problème avec ma chambre."
    hint: '« malheureusement » se place juste après le verbe conjugué.'
  - type: 'write'
    prompt: 'Traduis : « La calefacción no funciona desde esta mañana. »'
    answer: 'Le chauffage ne fonctionne plus depuis ce matin.'
    hint: '« ne...plus » = déjà plus (ya no); « depuis » = desde.'
  - type: 'order'
    sentence: "Pourriez-vous envoyer quelqu'un pour vérifier le problème"
    translation: '¿Podría enviar a alguien a revisar el problema?'
vocabulary:
  - term: "Excusez-moi, j'ai malheureusement un problème avec ma chambre."
    translation: 'Disculpe, lamentablemente tengo un problema con mi habitación.'
  - term: 'Le chauffage ne fonctionne plus depuis ce matin.'
    translation: 'La calefacción no funciona desde esta mañana.'
  - term: "Pourriez-vous envoyer quelqu'un pour vérifier le problème ?"
    translation: '¿Podría enviar a alguien a revisar el problema?'
  - term: "J'aimerais une autre chambre, si c'était possible."
    translation: 'Me gustaría otra habitación, si fuera posible.'
  - term: "Merci de votre compréhension — j'espère que ça pourra se résoudre vite."
    translation: 'Gracias por su comprensión — espero que se resuelva rápido.'
---

Tu es à la réception d'un hôtel. Ta chambre a un problème et tu dois le signaler sans
paraître brusque. En français, l'outil qui porte presque toute la politesse ici n'est pas
un mot spécial — c'est un temps verbal complet : **l'imparfait après « si »**.

## Les 5 blocs dont tu as besoin

Ne mémorise pas des mots isolés — mémorise ces phrases complètes, prêtes à l'emploi :

| Français                                                                | Español                                                        |
| ----------------------------------------------------------------------- | -------------------------------------------------------------- |
| Excusez-moi, j'ai malheureusement un problème avec ma chambre.          | Disculpe, lamentablemente tengo un problema con mi habitación. |
| Le chauffage ne fonctionne plus depuis ce matin.                        | La calefacción no funciona desde esta mañana.                  |
| Pourriez-vous envoyer quelqu'un pour vérifier le problème ?             | ¿Podría enviar a alguien a revisar el problema?                |
| J'aimerais une autre chambre, si c'était possible.                      | Me gustaría otra habitación, si fuera posible.                 |
| Merci de votre compréhension — j'espère que ça pourra se résoudre vite. | Gracias por su comprensión — espero que se resuelva rápido.    |

## La conversation complète

> **Réceptionniste :** Bonjour, comment puis-je vous aider ?
> **Client :** Excusez-moi, j'ai malheureusement un problème avec ma chambre. Le
> chauffage ne fonctionne plus depuis ce matin.
> **Réceptionniste :** Je suis désolée. J'envoie quelqu'un tout de suite.
> **Client :** J'aimerais une autre chambre, si c'était possible.
> **Réceptionniste :** Ce serait possible, si nous avions une chambre libre — mais
> l'hôtel est complet aujourd'hui.

## Découvre-le toi-même : pourquoi « si c'était » et pas « si c'est » ?

Regarde **« si c'**était** possible »** et **« si nous **avions** une chambre »**.
Compare ces formes avec le présent normal que tu connais déjà (**c'est**, **nous
avons**). Qu'est-ce que tu reconnais dans la terminaison de « étais/avions » ?

Demande-toi : est-ce que la chambre libre existe vraiment en ce moment, ou est-ce que la
réceptionniste adoucit un « non » ? La réponse est dans le verbe, pas dans le vocabulaire.

C'est la règle de **« si + imparfait »** : ce n'est pas une histoire au passé, c'est une
manière d'exprimer :

1. **La politesse** — « j'aimerais... si c'était possible » est bien plus doux que « je
   veux » tout court.
2. **L'hypothèse** — « si nous avions une chambre » sous-entend que ce n'est probablement
   pas le cas maintenant. C'est la même logique qu'en espagnol avec « si tuviéramos ».

## Pratique à voix haute (shadowing)

Lis toute la conversation trois fois :

1. **Lentement**, en marquant bien les liaisons (« vous_avez », « nous_avions »).
2. **À vitesse naturelle**, avec une légère montée de la voix sur « ...si c'était
   possible », comme une question douce et polie, pas une exigence.
3. **En t'enregistrant**, en jouant le rôle du client comme si tu étais vraiment à la
   réception. Écris aussi 3-4 phrases sur un autre problème d'hôtel (wifi, bruit,
   propreté), en réutilisant au moins deux blocs ci-dessus, et garde le tout comme si tu
   allais l'envoyer à un locuteur natif pour correction.
