---
language: pt
level: b1
title: 'O pretérito imperfeito vs o pretérito perfeito: el fondo y la acción'
description: 'El dúo que todo hispanohablante confunde al principio en portugués — con la ventaja de que la lógica es casi idéntica a la del español.'
order: 2
unit: 1
grammarTopic: 'Pretérito imperfeito vs Pretérito perfeito'
funFact: "Buenas noticias: esta distinción es la MÁS fácil de las que vas a estudiar en los cinco idiomas de PolyLingua, porque el portugués la organiza exactamente igual que el español. Si sabes cuándo usar 'llovía' vs 'llovió', ya sabes cuándo usar 'chovia' vs 'choveu'."
minutes: 8
quiz:
  - question: "¿Cuál es correcta para describir el clima como telón de fondo: '___ quando eu cheguei' (llovía cuando llegué)?"
    options: ['Choveu', 'Chovia', 'Chove']
    answerIndex: 1
    explanation: "'Chovia' (imperfeito) describe el estado/fondo continuo, mientras 'cheguei' (pretérito perfeito) es la acción puntual que interrumpe ese fondo — exactamente como en español."
  - question: '¿Cuál tiempo se usa para hábitos o acciones repetidas en el pasado, sin un momento específico?'
    options: ['O pretérito perfeito', 'O pretérito imperfeito', 'O futuro']
    answerIndex: 1
    explanation: "'Quando eu era criança, jogava todos os dias' (cuando era niño, jugaba todos los días) usa imperfeito porque describe un hábito repetido."
  - question: '¿Cuál oración combina correctamente ambos tiempos?'
    options:
      [
        'Eu via TV quando o telefone tocou.',
        'Eu via TV quando o telefone tocava.',
        'Eu vi TV quando o telefone tocava.',
      ]
    answerIndex: 0
    explanation: "'via' (imperfeito, fondo continuo) + 'tocou' (pretérito perfeito, evento puntual que interrumpe) es la combinación clásica: 'Veía la tele cuando sonó el teléfono'."
exercises:
  - type: 'fill-blank'
    sentence: '___ quando eu cheguei.'
    answer: 'Chovia'
    accepted: ['chovia']
    hint: 'Imperfeito para describir el fondo/estado continuo (llovía).'
    translation: 'Llovía cuando llegué.'
    placeholder: 'Fazia calor'
  - type: 'fill-blank'
    sentence: 'Ontem, eu ___ o museu.'
    answer: 'visitei'
    accepted: ['Visitei']
    hint: 'Pretérito perfeito para un evento puntual y completado.'
    translation: 'Ayer, visité el museo.'
    placeholder: 'cheguei / liguei'
  - type: 'match'
    instructions: 'Empareja cada frase en portugués con su traducción al español.'
    pairs:
      - left: 'Fazia calor'
        right: 'Hacía calor'
      - left: 'Eu jogava futebol todo sábado'
        right: 'Yo jugaba fútbol todos los sábados'
      - left: 'Levantei, comi, saí'
        right: 'Me levanté, comí, salí'
      - left: 'O telefone tocou'
        right: 'El teléfono sonó'
      - left: 'Ela estava cansada'
        right: 'Ella estaba cansada'
  - type: 'write'
    prompt: "Escribe en portugués (pretérito imperfeito): 'Cuando yo era niño, jugaba fútbol todos los sábados.'"
    answer: 'Quando eu era criança, jogava futebol todo sábado.'
    accepted: []
    hint: 'era (imperfeito de ser) + jogava (imperfeito de hábito repetido).'
    placeholder: 'Quando eu tinha dez anos, eu lia todo dia.'
  - type: 'order'
    sentence: 'Eu lia quando você ligou'
    translation: 'Yo leía cuando tú llamaste.'
vocabulary:
  - term: 'chovia'
    translation: 'llovía'
  - term: 'visitei'
    translation: 'visité'
  - term: 'jogava'
    translation: 'jugaba'
  - term: 'tocou'
    translation: 'sonó'
  - term: 'estava cansada'
    translation: 'estaba cansada'
  - term: 'fazia calor'
    translation: 'hacía calor'
---

Esta es, de los cinco idiomas que puedes estudiar en PolyLingua, la distinción de pasado
más fácil de dominar — porque el portugués organiza sus dos pasados exactamente igual
que el español.

## La misma lógica que ya conoces

```
Chovia       quando    eu saí.
(fondo, imperfeito)      (acción puntual, pretérito perfeito)
```

Si en español dirías "llovía cuando salí" (no "llovió cuando salía"), tu intuición ya
funciona perfectamente en portugués.

## Cuándo usar el imperfeito

| Uso                                       | Ejemplo                          |
| ----------------------------------------- | -------------------------------- |
| Descripciones (clima, apariencia, estado) | Fazia calor. Ela estava cansada. |
| Hábitos/repetición sin límite claro       | Eu jogava futebol todo sábado.   |
| Fondo de una acción                       | Eu lia quando você ligou.        |

## Cuándo usar el pretérito perfeito

| Uso                             | Ejemplo                              |
| ------------------------------- | ------------------------------------ |
| Eventos puntuales y completados | Ontem, visitei o museu.              |
| Una serie de acciones concretas | Levantei, comi, saí.                 |
| Algo que interrumpe el fondo    | O telefone tocou enquanto eu dormia. |

## Cómo se forma el imperfeito (repaso rápido)

| Pronombre | falar (-ar) | comer (-er) | abrir (-ir) |
| --------- | ----------- | ----------- | ----------- |
| eu        | falava      | comia       | abria       |
| ele/ela   | falava      | comia       | abria       |
| nós       | falávamos   | comíamos    | abríamos    |

## El único aviso: los irregulares

Solo tres verbos son irregulares en imperfeito: ser (era), ter (tinha), vir (vinha). El
resto sigue el patrón regular sin sorpresas — otra ventaja frente a otros idiomas donde
esta distinción trae más irregularidades.
