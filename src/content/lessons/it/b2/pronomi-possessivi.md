---
language: it
level: b2
title: "I pronomi possessivi: il mio, il tuo, y sus primos"
description: "Cómo reemplazar 'mi libro' por 'el mío' sin repetir el sustantivo, manteniendo artículo y concordancia."
order: 6
grammarTopic: "Pronombres posesivos (il mio, il tuo, il suo...)"
funFact: "A diferencia de los adjetivos posesivos que ya conoces (donde el artículo desaparece con familiares singulares: 'mia madre'), los pronombres posesivos SIEMPRE mantienen su artículo, sin excepción: 'Questo libro è il mio' — nunca se omite, porque aquí el pronombre reemplaza completamente al sustantivo."
minutes: 8
quiz:
  - question: "'È il mio libro.' → 'È ___' (es el mío). ¿Qué forma va?"
    options: ["il mio", "mio", "il libro mio"]
    answerIndex: 0
    explanation: "El pronombre posesivo mantiene su artículo: È il mio (es el mío), reemplazando 'il mio libro' completo."
  - question: "¿Cuál es el pronombre posesivo correcto para 'la sua macchina' (su coche, de ella — femenino)?"
    options: ["il suo", "la sua", "le sue"]
    answerIndex: 1
    explanation: "'macchina' es femenino singular, así que el pronombre concuerda: la sua (la suya)."
  - question: "Los pronombres posesivos concuerdan en género y número con:"
    options: [
      "La persona que posee el objeto",
      "El objeto poseído",
      "No concuerdan con nada"
    ]
    answerIndex: 1
    explanation: "Al igual que los adjetivos posesivos, los pronombres concuerdan con el género y número de la COSA poseída, no con quién la posee."
exercises:
  - type: "fill-blank"
    sentence: "È il mio libro. → È ___."
    answer: "il mio"
    accepted: ["il mio"]
    hint: "El pronombre posesivo mantiene siempre su artículo."
    translation: "Es mi libro. → Es el mío."
  - type: "fill-blank"
    sentence: "È la sua macchina. → È ___."
    answer: "la sua"
    accepted: ["la sua"]
    hint: "'macchina' es femenino singular: el pronombre debe concordar."
    translation: "Es su coche. → Es el suyo."
  - type: "match"
    instructions: "Empareja cada poseedor con su pronombre posesivo (masculino singular)."
    pairs:
      - left: "io"
        right: "il mio"
      - left: "tu"
        right: "il tuo"
      - left: "lui/lei"
        right: "il suo"
      - left: "noi"
        right: "il nostro"
      - left: "voi"
        right: "il vostro"
  - type: "write"
    prompt: "Escribe el pronombre posesivo (masculino plural) correspondiente a 'noi' (los nuestros)."
    answer: "i nostri"
    accepted: ["I nostri"]
    hint: "Recuerda: masculino plural, poseedor 'noi'."
  - type: "order"
    sentence: "Questo libro è il mio"
    translation: "Este libro es el mío."
---

Estos pronombres reemplazan completamente un sustantivo poseído — manteniendo siempre
su artículo, a diferencia de la excepción que viste con familiares singulares.

## La tabla completa

| Poseedor | Masc. sing. | Fem. sing. | Masc. plural | Fem. plural |
| --- | --- | --- | --- | --- |
| io | il mio | la mia | i miei | le mie |
| tu | il tuo | la tua | i tuoi | le tue |
| lui/lei | il suo | la sua | i suoi | le sue |
| noi | il nostro | la nostra | i nostri | le nostre |
| voi | il vostro | la vostra | i vostri | le vostre |
| loro | il loro | la loro | i loro | le loro |

## Reemplazando en contexto

```
È il mio libro. → È il mio.
È la sua macchina. → È la sua.
Sono le nostre idee. → Sono le nostre.
```

## Sin excepciones (a diferencia del adjetivo posesivo)

Recuerda que "mia madre" (sin artículo) es una excepción de los ADJETIVOS posesivos
con familiares singulares. Los PRONOMBRES posesivos siempre mantienen su artículo, sin
esa excepción:

```
Adjetivo:    mia madre           (sin artículo — excepción con familiar singular)
Pronombre:    Questa è la mia.     (con artículo — el pronombre siempre lo lleva)
```

## Comparando adjetivo posesivo vs. pronombre posesivo

```
È il mio libro.        (adjetivo, antes de sustantivo)
Questo libro è il mio.   (pronombre, reemplaza completamente al sustantivo)
```

## El truco práctico

El artículo (il/la/i/le) es parte inseparable del pronombre posesivo — nunca lo
omitas, ni siquiera con familiares. Piensa en "il mio/la mia" como una sola unidad
compuesta que ya incluye tanto el artículo como la concordancia.
