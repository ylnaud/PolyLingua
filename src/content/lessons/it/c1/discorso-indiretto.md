---
language: it
level: c1
title: "Il discorso indiretto: cuenta lo que otros dijeron, con precisión"
description: "Cómo cambian los tiempos verbales, los pronombres y las expresiones de tiempo al reportar las palabras de alguien más."
order: 2
grammarTopic: "Discorso indiretto (concordanza dei tempi)"
funFact: "Igual que en francés o inglés, el italiano retrocede sistemáticamente los tiempos verbales al reportar discurso: presente → imperfetto, passato prossimo → trapassato prossimo, futuro → condizionale passato. Es un movimiento mecánico de 'un paso hacia atrás' que, una vez automatizado, se vuelve instantáneo."
minutes: 10
quiz:
  - question: "Maria ha detto: 'Sono stanca.' ¿Cómo lo reportas?"
    options: [
      "Maria ha detto che è stanca.",
      "Maria ha detto che era stanca.",
      "Maria ha detto che sarà stanca."
    ]
    answerIndex: 1
    explanation: "El presente ('sono') retrocede a imperfetto ('era') al reportar el discurso: Maria ha detto che era stanca."
  - question: "Paolo ha detto: 'Verrò domani.' ¿Cómo lo reportas?"
    options: [
      "Paolo ha detto che verrà domani.",
      "Paolo ha detto che sarebbe venuto il giorno dopo.",
      "Paolo ha detto che viene domani."
    ]
    answerIndex: 1
    explanation: "'verrò' (futuro) retrocede a 'sarebbe venuto' (condizionale passato), y 'domani' cambia a 'il giorno dopo' porque el punto de referencia temporal cambió."
  - question: "¿Qué le pasa a la palabra 'domani' (mañana) al reportar discurso en pasado?"
    options: [
      "Se queda exactamente igual",
      "Cambia a 'il giorno dopo' (al día siguiente)",
      "Desaparece de la frase"
    ]
    answerIndex: 1
    explanation: "Los marcadores de tiempo cambian junto con los verbos: domani → il giorno dopo, oggi → quel giorno, ieri → il giorno prima."
exercises:
  - type: "fill-blank"
    sentence: "Maria ha detto che ___ stanca."
    answer: "era"
    accepted: ["era"]
    hint: "El presente 'sono' retrocede a imperfetto 'era' en discurso indirecto."
    translation: "Maria dijo que estaba cansada."
  - type: "fill-blank"
    sentence: "Paolo ha detto che sarebbe venuto il giorno ___."
    answer: "dopo"
    accepted: ["dopo"]
    hint: "'domani' cambia a 'il giorno dopo' al reportar discurso."
    translation: "Paolo dijo que vendría al día siguiente."
  - type: "match"
    instructions: "Empareja cada marcador de discurso directo con su equivalente en discurso indirecto."
    pairs:
      - left: "oggi"
        right: "quel giorno"
      - left: "domani"
        right: "il giorno dopo"
      - left: "ieri"
        right: "il giorno prima"
      - left: "adesso"
        right: "in quel momento"
  - type: "write"
    prompt: "Reporta en discurso indirecto lo que dijo Paolo: 'Ho finito.'"
    answer: "Paolo ha detto che aveva finito."
    accepted: ["Paolo ha detto che aveva finito"]
    hint: "El passato prossimo retrocede a trapassato prossimo: aveva finito."
  - type: "order"
    sentence: "Mi ha detto di chiudere la porta"
    translation: "Me dijo que cerrara la puerta."
---

Cuando cuentas lo que alguien más dijo, el italiano no usa comillas — cambia
sistemáticamente los tiempos verbales, un fenómeno llamado **concordanza dei tempi**.

## La regla del retroceso

| Discurso diretto | Discorso indiretto |
| --- | --- |
| Presente | Imperfetto |
| Passato prossimo | Trapassato prossimo |
| Futuro | Condizionale passato |
| Imperativo | di + infinito |

```
Diretto:      "Sono stanca."        →  Indiretto: Ha detto che era stanca.
Diretto:      "Ho finito."           →  Indiretto: Ha detto che aveva finito.
Diretto:      "Verrò."               →  Indiretto: Ha detto che sarebbe venuto.
```

## Los marcadores de tiempo también cambian

| Diretto | Indiretto |
| --- | --- |
| oggi | quel giorno |
| domani | il giorno dopo |
| ieri | il giorno prima |
| adesso | in quel momento |

```
Diretto:    "Parto domani."
Indiretto:  Ha detto che partiva il giorno dopo.
```

## El caso especial del imperativo

Las órdenes en discurso directo se transforman en "di + infinito":

```
Diretto:      "Chiudi la porta!"
Indiretto:    Mi ha detto di chiudere la porta.
```

## Por qué es un reto de nivel C1

Aplicar correctamente esta doble transformación —tiempo verbal Y marcadores
temporales— de forma simultánea y fluida, sin pausar a pensar, es exactamente el tipo de
automatismo que separa a un hablante C1 de uno que todavía traduce mentalmente frase por
frase.
