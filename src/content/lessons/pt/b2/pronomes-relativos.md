---
language: pt
level: b2
title: "Os pronomes relativos: la clave para frases más elegantes"
description: "Que, quem, cujo y onde: los pronombres relativos que conectan dos ideas en una sola frase fluida, sin repetir el sustantivo en portugués."
order: 2
grammarTopic: "Pronomes relativos (que, quem, cujo, onde)"
funFact: "'Que' es el pronombre relativo más versátil del portugués — funciona para sujeto y objeto, personas y cosas. El verdadero reto está en 'cujo', que no se traduce como una sola palabra en español: significa 'cuyo/cuya' pero concuerda en género y número con lo que sigue, no con el poseedor."
minutes: 9
quiz:
  - question: "'Tenho um amigo. Esse amigo mora em Lisboa.' → 'Tenho um amigo ___ mora em Lisboa.' ¿Qué pronombre va?"
    options: ["quem", "que", "cujo"]
    answerIndex: 1
    explanation: "'que' es el pronombre relativo por defecto, tanto para sujeto como objeto: Tenho um amigo que mora em Lisboa."
  - question: "'Este é o autor. O livro DELE ganhou um prêmio.' → 'Este é o autor ___ livro ganhou um prêmio.' ¿Qué pronombre va?"
    options: ["que", "cujo", "onde"]
    answerIndex: 1
    explanation: "'cujo' expresa posesión (equivale a 'cuyo'): Este é o autor cujo livro ganhou um prêmio. Concuerda con 'livro' (masculino), no con 'autor'."
  - question: "¿Con qué concuerda 'cujo/cuja/cujos/cujas' en género y número?"
    options: [
      "Con el poseedor (la persona)",
      "Con la cosa poseída (lo que sigue inmediatamente después)",
      "Siempre se queda en masculino singular"
    ]
    answerIndex: 1
    explanation: "A diferencia de lo que uno esperaría, 'cujo' concuerda con el sustantivo que sigue (la cosa poseída), no con el poseedor: a autora cujo LIVRO (masc.) vs. o autor cuja HISTÓRIA (fem.)."
exercises:
  - type: "fill-blank"
    sentence: "Tenho um amigo ___ mora em Lisboa."
    answer: "que"
    hint: "'que' es el pronombre relativo por defecto."
    translation: "Tengo un amigo que vive en Lisboa."
    placeholder: "que / quem / onde"
  - type: "fill-blank"
    sentence: "Este é o autor ___ livro ganhou um prêmio."
    answer: "cujo"
    hint: "Concuerda con 'livro' (masculino), no con 'autor'."
    translation: "Este es el autor cuyo libro ganó un premio."
    placeholder: "que / cuja / onde"
  - type: "match"
    instructions: "Empareja cada pronombre relativo con su uso."
    pairs:
      - left: "que"
        right: "sujeto u objeto, personas y cosas"
      - left: "quem"
        right: "personas, tras preposición"
      - left: "cujo/cuja"
        right: "posesión, concuerda con lo poseído"
      - left: "onde"
        right: "lugar"
  - type: "write"
    prompt: "Escribe en portugués: 'La ciudad donde nací es bonita.'"
    answer: "A cidade onde eu nasci é linda."
    hint: "'onde' introduce el lugar."
    placeholder: "A cidade onde + sujeito + verbo + adjetivo."
  - type: "order"
    sentence: "A mulher que fala é minha irmã"
    translation: "La mujer que habla es mi hermana."
vocabulary:
  - term: "que"
    translation: "que"
  - term: "quem"
    translation: "quien"
  - term: "cujo"
    translation: "cuyo"
  - term: "cuja"
    translation: "cuya"
  - term: "onde"
    translation: "donde"
---

Los pronombres relativos son la herramienta que separa el portugués de principiante
("Tenho um amigo. Ele mora em Lisboa.") del portugués fluido ("Tenho um amigo que mora em
Lisboa.").

## "que": el pronombre que hace casi todo el trabajo

Al igual que en italiano, "que" funciona para sujeto y objeto directo, personas y cosas:

| Función | Ejemplo |
| --- | --- |
| Sujeto | A mulher que fala é minha irmã. |
| Objeto directo | O filme que eu vi era ótimo. |

## "quem": solo para personas, después de preposición

```
A pessoa com quem eu falei...    (la persona con quien hablé)
O amigo a quem escrevi...         (el amigo a quien escribí)
```

## "cujo/cuja/cujos/cujas": el pronombre posesivo que sorprende

Este es el verdadero reto: "cujo" significa "cuyo", pero **concuerda con lo que sigue**,
no con el poseedor:

```
o autor cujo LIVRO ganhou um prêmio     (livro es masculino → cujo)
a autora cuja HISTÓRIA emocionou todos   (história es femenino → cuja)
```

## "onde": lugar

```
A cidade onde eu nasci é linda.    (la ciudad donde nací es bonita)
```

## Combinándolos en una frase natural

```
Esta é a casa    onde    cresci,    cujo    jardim    eu adorava,    e    que    sinto falta.
(esta es la casa donde crecí, cuyo jardín adoraba, y que extraño)
```

Cuantos más pronombres relativos domines con soltura —especialmente el traicionero
"cujo"—, menos frases cortas y entrecortadas tendrás: justo el salto de calidad que
define el nivel B2.
